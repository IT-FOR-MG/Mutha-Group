...
public class ODK_IT_Tracker_DAO {
	boolean flag = false;

	public boolean getMaxReqId(ODK_IT_Tracker_VO vo){
		Connection con = ConnectionModel.getConnection();
		PreparedStatement reqid;
		try {
			reqid = con.prepareStatement("select max(U_Req_Id) from it_user_requisition");
			ResultSet rs_reqid= reqid.executeQuery();
			if(rs_reqid.next())
			{
				vo.setReq_no(rs_reqid.getInt(1)+1);
			}
	...
	public boolean addReq(ODK_IT_Tracker_VO vo) {
		try {
			int uid = 0;
			int comp_id = 0;
			uid = vo.getUid();
